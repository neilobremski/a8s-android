...
| `SmsSegmenter.kt` | Splits output above the configured character limit (default 1000) into word/URL-safe logical chunks labeled `Message <id> part <N> of <M>:`. Each chunk is then passed whole to `SmsManager.sendMultipartTextMessage`, so Android reassembles its carrier parts into one message bubble. |
