...
| `IncomingSmsRouter.kt` | SMS/RCS ingress: phone-principal match, SMS-originated slash commands, fall-through publish to the sticky target as phone agent. Role-denied commands are logged and silently dropped. Inbound upload failures publish their error and trigger one sender-facing alert. The sticky pin (`SharedPreferences`, keyed by phone-principal agent) carries a timestamp alongside the target; every read-then-write goes through one pin lock, and a legacy value with no stored timestamp is stamped once on first inbound observation and ages normally from there. A plain-text fall-through refreshes the timestamp only while the pin still points at the target it routed to. |
| `SmsSlashCommand.kt` | Classify inbound SMS bodies as authorized/forbidden/conversational-tell/not-a-command using role policy. `hey`/`ok`/`okay <agent> ...` classify as `ConversationalTell` after the same `tell` permission gate; the router resolves the target (`NicknamesManager.resolveTell` + `NicknamesManager.isKnownTarget`) before dispatching, and falls through to sticky routing when it doesn't resolve — `/tell` itself has no such guard. |
| `SmsCommandDelivery.kt` | Phone-agent SMS forward + SMS reply body building (inline storage URLs). After the dedup gate passes, an inbound forward re-pins the sticky target to its sender only when the phone principal's pin is absent or stale (`StickyPin.shouldRepin`, gated by `settings.sms_sticky_ttl_s`); a fresh pin is left alone. |
| `StickyPin.kt` | Pure staleness rule for the sticky SMS target — `shouldRepin(nowMs, pinnedAtMs, ttlMs)`. `ttlMs == 0` disables expiry. No Android types, so it tests without a Context. |
...
| `CmdNicknames.kt` / `NicknamesManager.kt` | Nickname command handler + SharedPreferences store. Mapping and enabled-state preferences are separate; direct `/tell` targets strip surrounding spoken punctuation, and targets/resolved destinations normalize to lowercase. Known canonical device/principal names cannot be registered or overridden by nickname mappings. |
