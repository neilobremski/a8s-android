...
object CmdLongtap {
    ...
    fun run(service: A8sService, config: A8sAndroid.Config, cmd: MqttRoute.Command) {
        ...
        if (cmd.args.size < 2) {
            service.replyToSender(config, cmd.sender, "Usage: /longtap x y [ms]")
            return
        }
        val x = cmd.args[0].toIntOrNull()
        val y = cmd.args[1].toIntOrNull()
        if (x == null || y == null) {
            service.replyToSender(config, cmd.sender, "Usage: /longtap x y [ms] (integers)")
            return
        }
        ...
        UiActionReply.send(service, config, cmd.sender, text, "longtap")
    }
}
