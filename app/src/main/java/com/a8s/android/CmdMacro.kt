...
object CmdMacro {
    ...
    fun run(service: A8sService, config: A8sAndroid.Config, cmd: MqttRoute.Command) {
        ...
        if (raw.isEmpty()) {
            service.replyToOwner(config, cmd.owner, "Usage: /macro <step1> | <step2> | …")
            return
        }
        val steps = MacroParser.parse(raw)
        if (steps.isEmpty()) {
            service.replyToOwner(config, cmd.owner, "/macro: no steps parsed from input")
            return
        }
        ...
        if (!service.captureScreenshotPng(before)) {
            service.replyToOwner(config, cmd.owner, "Macro failed: 'before' screenshot capture failed")
            return
        }

        val recorderState = startRecording(service, recording)
        if (recorderState == null) {
            service.replyToOwner(config, cmd.owner, "Macro failed: screen recorder could not start")
            return
        }
        ...
        service.replyToOwner(config, cmd.owner, body, files = files)
    }
