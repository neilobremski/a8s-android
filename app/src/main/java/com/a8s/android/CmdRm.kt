...
object CmdRm {
    ...
    fun run(service: A8sService, config: A8sAndroid.Config, cmd: MqttRoute.Command) {
        ...
        if (pathArg == null) {
            service.replyToSender(config, cmd, "Usage: /rm <path>")
            return
        }
        try {
            val target = File(pathArg)
            if (!target.exists()) {
                service.replyToSender(config, cmd, "rm: $pathArg: no such file or directory")
                return
            }
            if (target.isDirectory && (target.listFiles()?.isNotEmpty() == true)) {
                service.replyToSender(config, cmd, "rm: $pathArg: directory not empty")
                return
            }
            val ok = target.delete()
            if (!ok) {
                service.replyToSender(config, cmd, "rm: $pathArg: delete failed")
                return
            }
            service.replyToSender(config, cmd, "rm: $pathArg: deleted")
        } catch (e: Exception) {
            A8sAndroid.log("rm failed: ${e.message}")
            service.replyToSender(config, cmd, "rm failed: ${e.message}")
        }
    }
}
