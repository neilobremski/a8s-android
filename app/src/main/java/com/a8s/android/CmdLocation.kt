...
object CmdLocation {
    ...
    fun run(service: A8sService, config: A8sAndroid.Config, cmd: MqttRoute.Command) {
        ...
        if (lm == null) {
            service.replyToOwner(config, cmd.owner, "Location failed: LocationManager unavailable")
            return
        }
        ...
        if (location == null) {
            service.replyToOwner(config, cmd.owner, "Location failed: no fix within ${ONE_SHOT_TIMEOUT_S}s")
            return
        }
        ...
        service.replyToOwner(config, cmd.owner, CmdHelpers.renderLocation(snap))
    }
