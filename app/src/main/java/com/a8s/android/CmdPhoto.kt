...
object CmdPhoto {
    ...
    fun run(service: A8sService, config: A8sAndroid.Config, cmd: MqttRoute.Command) {
        ...
        if (mgr == null) {
            service.replyToSender(config, cmd, "Photo failed: camera service unavailable")
            return
        }
        val cameraId = findCameraId(mgr, facing)
        if (cameraId == null) {
            service.replyToSender(config, cmd, "Photo failed: no $facing camera found")
            return
        }
        ...
        try {
            ...
            if (!ok || dest.length() == 0L) {
                service.replyToSender(config, cmd, "Photo failed: no image captured")
                return
            }
            ...
            service.replyToSender(config, cmd, "Photo failed: ${e.message}")
        } finally {
            thread.quitSafely()
        }
    }
