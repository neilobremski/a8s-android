...
object CmdSwipe {
    ...
    fun run(service: A8sService, config: A8sAndroid.Config, cmd: MqttRoute.Command) {
        ...
        if (cmd.args.size < 4) {
            service.replyToSender(config, cmd.sender, "Usage: /swipe x1 y1 x2 y2 [ms]")
            return
        }
        val coords = (0..3).map { cmd.args[it].toIntOrNull() }
        if (coords.any { it == null }) {
            service.replyToSender(config, cmd.sender, "Usage: /swipe x1 y1 x2 y2 [ms] (integers)")
            return
        }
        val x1 = coords[0]!!
        val y1 = coords[1]!!
        ...
        UiActionReply.send(service, config, cmd.sender, text, "swipe")
    }
}
